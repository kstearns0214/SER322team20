...
SELECT name,step,text
FROM recipe 
INNER JOIN instructions
ON recipe.recipeID = instructions.recipeID;

SELECT recipe.name AS recipeName, has_ingredients.name AS ingredientName, amount, unitOfMeasure
...
Select COUNT(has_ingredients.name) AS NumberOfIngredients
FROM has_ingredients
INNER JOIN recipe
ON recipe.recipeID = has_ingredients.recipeID
WHERE recipe.name = 'Omellete';
